...
local notes = require('scene-conducting/notes')
...
function sampler.new(settings)
  ...
  for i,sample in ipairs(settings) do
    sample.soundData = lovr.data.newSoundData(sample.path)
...
function sampler:assignSynth(touchId, touch)
  ...
  synth.source = lovr.audio.newSource(sample.soundData)
  synth.source:setFalloff(1, 50, 1)
  reference, max, rolloff = synth.source:getFalloff(1, 50, 1)
  ...
  if touch.location then
    synth.source:setPosition(unpack(touch.location))
  end
